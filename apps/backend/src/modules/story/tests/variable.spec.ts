...
describe('VariableService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get(VariableService);
    repository = module.get(VariableRepository);
  ...
  describe('findAllByStory', () => {
    it('should delegate to repository without checking editability', async () => {
      const variables = [{ key: 'gold' }];
      repository.findAllByStory.mockResolvedValue(variables);
  ...
  describe('delete', () => {
    it('should check editability before deleting', async () => {
      ...
      const result = await service.delete('story-1', 'gold', 'author-1');
